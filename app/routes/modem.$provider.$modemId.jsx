...
export const loader = async ({ request }) => {
	try {
		const modemDataPromise = modemLoader({ request });
		return defer({
			modemData: modemDataPromise,
		});
	} catch (error) {
		console.error('Error loading modem data:', error);
		throw new Error('Failed to load modem data');
	}
};
		...
		<Layout>
			...
			<main className='content'>
				...
				>
					<Await
						resolve={modemData}
						errorElement={<div className='error-container'>Error loading modem data</div>}
					>
						{(resolvedData) => {
							const { modem, gpsData, latencyData, throughputData, signalQualityData, obstructionData, usageData, uptimeData, mapsAPIKey } = resolvedData;
